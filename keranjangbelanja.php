<div class="container5">
	...
	$sqlc =  mysqli_query($kon, "select * from cart where idanggota='$_SESSION[idanggota]'");
	...
	while($rc = mysqli_fetch_array($sqlc)){
		...
		echo "<td>
					...
					<br>
					$diskon $hargalama<br>
					<big>IDR $hrgbr * <span class='jmlbeli'><input type='number' name='jml[$no]' value='$rc[jumlahbeli]'  style='text-align:left'></span> Unit</big>
					</td>";
		echo "</tr>";
		echo "</table>";
		echo "</div>";
	...
	echo "<br>";
	echo "<br>";
	echo "<a href='?p=keranjangdel&idc=$rc[idcart]&idag=$rag[idanggota]'><button type='button' class='btn-x'>Delete</button></a>";
